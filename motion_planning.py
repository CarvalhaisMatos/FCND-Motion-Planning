...
class MotionPlanning(Drone):
  ...
  def __init__(self, connection, goal_global):
    ...
    self.goal_global_position = goal_global
  ...
  def get_home(self):
    # max_rows did not worked on this version of numpy, and updating have the risk of errors in future code
    with open('colliders.csv') as f:
      lat0, lon0 = f.readlines()[0].split(",")

    lat0 = float(lat0.split('lat0')[1].lstrip().strip())
    lon0 = float(lon0.split('lon0')[1].lstrip().strip())
    return lat0, lon0
  ...
  def plan_path(self):
    ...
    start_north = int(np.ceil(local_north - north_offset))
    start_east = int(np.ceil(local_east - east_offset))
...
if __name__ == "__main__":
  ...
  goal_global = np.fromstring('{},{},{}'.format(args.goal_lon, args.goal_lat, args.goal_alt), dtype='Float64', sep=',')
  drone = MotionPlanning(conn, goal_global=goal_global)
  time.sleep(1)
